...
export default {
  ...
  render() {
    const thead = this.hideHeader ? null : (<thead style="background:#F6F7FB;opacity:0.5;font-size:14px;color:#999999;font-family: MicrosoftYaHei;">
      {
        this.weekDays.map(day => <th key={day}>{ `星期${day}` }</th>)
      }
    </thead>);
    return (
      <table
            ...
            this.rows.map((row, index) => <tr
              ...
              key={index}>
              {
                row.map((cell, key) => <td key={key}
                  ...
                  <div class="el-calendar-day" style="width:100%;min-width: 111px;height:125px">
                    {
